...
export class AppStoreComponent implements OnInit{
  ...
  async onScroll(event: any) {
    if (this.bottomReached() && !this.loading) {
      if(this.queryBoolean){
        this.productList = [...await this.paginationService.getNextquery(this.coleccion,this.field,this.queryText)];
      }else{
  ...
  async queryFunc(coleccion:string,field:string,query:string){
    this.productList = [...await this.paginationService.getFirstquery(coleccion,field,query)];
